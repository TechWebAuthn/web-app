export class WebAuthnConnect extends HTMLElement {
  ...
  connectedCallback() {
    ...
    this.addEventListener("connection-request-registration-token", this._onRequestRegisterAddTokenListener);
  }

  disconnectedCallback() {
    this.root.querySelector("form").removeEventListener("submit", this._onFormSubmitListener);
    this.removeEventListener("connection-request-registration-token", this._onRequestRegisterAddTokenListener);
  }
  ...
  async _onRequestRegisterAddToken() {
    try {
      ...
      this.dispatchEvent(new CustomEvent("connection-finished", { detail: jsonRegistrationResponse }));
    } catch (error) {
      this.dispatchEvent(new CustomEvent("connection-error", { detail: { message: error.message } }));
    }
